...
interface MostBookedService extends Service {
  booking_count: number;
}

export function MostBookedSection({ services, loading }: { services: MostBookedService[]; loading: boolean }) {
  if (loading) {
    return (
    ...
    <section className="py-8 md:py-16">
      <div className="container mx-auto px-4">
        <div className="flex items-center gap-4 mb-4 md:mb-6">
          <h2 className="text-xl md:text-3xl font-bold">Los Mas Solicitados</h2>
        ...
        <HorizontalCarousel>
          {services.map(svc => (
            <div key={svc.id} className="min-w-[44vw] max-w-[48vw] md:min-w-[220px] md:max-w-[260px] snap-start flex-shrink-0">
              <ServiceCarouselCard
                service={svc}
                badge={{ text: `${svc.booking_count} contrataciones`, className: 'bg-amber-100 text-amber-800' }}
              />
            </div>
          ))}
        </HorizontalCarousel>
