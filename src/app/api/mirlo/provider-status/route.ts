...
export async function GET(request: NextRequest) {
  ...
  // Find provider profile by phone
  ...
  return NextResponse.json({
    ...
    has_services: (serviceCount ?? 0) > 0,
    service_count: serviceCount ?? 0,
    active_service_count: activeServiceCount ?? 0,
    has_bookings: (totalBookings ?? 0) > 0,
    total_bookings: totalBookings ?? 0,
    registration_date: profile.created_at,
    has_fiscal_data: !!fiscal,
    fiscal_status: fiscal?.fiscal_status ?? null,
  });
}
